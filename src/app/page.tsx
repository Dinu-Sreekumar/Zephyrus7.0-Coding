...
export default function Home() {
  ...
  const handleDownloadSummary = useCallback((finalSubmissions: Submission[]) => {
    ...
    problems.forEach((problem, index) => {
        ...
        fileContent += `========================================\n\n`;
        if (sub) {
            fileContent += `Language: ${sub.language}\n`;
            fileContent += `Time Remaining at last save: ${formatTime(sub.timeSubmitted)}\n\n`;
            fileContent += `--- Code ---\n`;
            fileContent += `${sub.code || 'No code submitted.'}\n\n`;
            fileContent += `--- Output ---\n`;
            fileContent += `${sub.output || 'No output generated.'}\n\n\n`;
        } else {
    ...
    a.href = url;
    a.download = `zephyrus_coding_event_summary_${userName.replace(/\s+/g, '_')}_${chestNumber}.txt`;
  ...
  const handleFinish = () => {
    ...
    handleDownloadSummary(submissions);
    setGameState('finished');
  }
